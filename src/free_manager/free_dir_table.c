...
PageOffset FreeDirGetPageSize(FreeDirTable* free_dir_table) {
...
void FreeDirTableInit(FreeDirTable* free_dir_table, uint32_t level, PageOffset page_size) {
    ...
    PageCount sub_max_free_page = FreeTableGetPageCount(level, page_size);
    ...
    // 当初始化目录表时，当前dir_table页就应该在page_table被分配
