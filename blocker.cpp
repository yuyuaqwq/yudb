...
std::optional<std::pair<uint16_t, PageOffset>> Blocker::BlockAlloc(PageSize size, BlockRecord* record_arr) {
    ...
    if (!record_arr) {
        alloc_new_page = true;
        record_arr = reinterpret_cast<BlockRecord*>(&record_cache.data[block_info_->record_offset]);
    }
...
void Blocker::BlockInfoBuild() {
    ...
    BlockRecordBuild(&record_arr[0], &record_page, sizeof(BlockRecord));
}
