...
__forceinline YuDbBPlusLeafListEntry* YuDbBPlusLeafEntryReferencer_Reference(YuDbBPlusLeafListHead* head, PageId entry_id) {
	YuDbBPlusTree* tree = ((YuDbBPlusTree*)((uintptr_t)(head)-((uintptr_t) & (((YuDbBPlusTree*)0)->leaf_list))));
...
__forceinline void YuDbBPlusLeafEntryReferencer_Dereference(YuDbBPlusLeafListHead* head, YuDbBPlusLeafListEntry* list_entry) {
	YuDbBPlusTree* tree = ((YuDbBPlusTree*)((uintptr_t)(head)-((uintptr_t) & (((YuDbBPlusTree*)0)->leaf_list))));
	YuDbBPlusLeafEntry* entry = ((YuDbBPlusLeafEntry*)((uintptr_t)(list_entry)-((uintptr_t) & (((YuDbBPlusLeafEntry*)0)->list_entry))));
...
void YuDbBPlusLeafListReplaceEntry(YuDbBPlusLeafListHead* head, PageId entry_id, PageId new_entry_id) {
	...
	YuDbBPlusTree* tree = ((YuDbBPlusTree*)((uintptr_t)(head)-((uintptr_t) & (((YuDbBPlusTree*)0)->leaf_list))));
	YuDbBPlusEntry* entry_entry = YUDB_BUCKET_BPLUS_REFERENCER_Reference(tree, entry_id);
	YuDbBPlusEntry* new_entry_entry = YUDB_BUCKET_BPLUS_REFERENCER_Reference(tree, new_entry_id);
	assert(entry_entry->type == kBPlusEntryLeaf);
	assert(new_entry_entry->type == kBPlusEntryLeaf);
	YUDB_BUCKET_BPLUS_REFERENCER_Dereference(tree, entry_entry);
	YUDB_BUCKET_BPLUS_REFERENCER_Dereference(tree, new_entry_entry);
	PageId prev_id = entry->prev;
	PageId next_id = entry->next;
	if (prev_id == entry_id) {
	...
	YuDbBPlusEntry* next_entry = YUDB_BUCKET_BPLUS_REFERENCER_Reference(tree, next_id);
	YuDbBPlusEntry* prev_entry = YUDB_BUCKET_BPLUS_REFERENCER_Reference(tree, prev_id);
	assert(prev_entry->type == kBPlusEntryLeaf);
	assert(next_entry->type == kBPlusEntryLeaf);
	YUDB_BUCKET_BPLUS_REFERENCER_Dereference(tree, prev_entry);
	YUDB_BUCKET_BPLUS_REFERENCER_Dereference(tree, next_entry);
...
_Bool YuDbBPlusEntryRbBsTreeInsert(YuDbBPlusEntryRbBsTree* tree, int16_t entry_id) {
	...
	while (cur_id != YuDbBPlusEntryRbReferencer_InvalidId) {
		...
		int32_t* entry_key = YUDB_BUCKET_BPLUS_RB_TREE_ACCESSOR_GetKey(tree, entry);
...
BPlusCursorStatus YuDbBPlusCursorNext(YuDbBPlusTree* tree, YuDbBPlusCursor* cursor, int32_t* key) {
	...
	if (i == 64517) {
		PrintRB(&cur_entry->rb_tree, cur_entry->rb_tree.root, 0, true);
		printf("\n\n\n\n\n\n\n\n");
	}
	int8_t cmp_status = -1;
...
static _Bool YuDbBPlusTreeDeleteElement(YuDbBPlusTree* tree, YuDbBPlusCursor* cursor) {
	...
	do {
		...
		if (sibling->element_count > (m - 1) * 10 / 25) {
			if (entry->type == kBPlusEntryLeaf) {
				if (left_sibling) {
