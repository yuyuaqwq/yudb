...
ptrdiff_t CacheLruListHashLinkVectorPushMultipleTail(CacheLruListHashLinkVector* arr, const CacheLruListHashLinkEntry* obj, size_t count) {
	if (arr->capacity <= arr->count + count) {
		CacheLruListHashLinkVectorExpand(arr, count);
	}
	memcpy((void*)(&arr->obj_arr[arr->count]), (void*)(obj), (sizeof(CacheLruListHashLinkEntry) * count));
	arr->count += count;
...
ptrdiff_t CacheLruListHashBucketVectorPushMultipleTail(CacheLruListHashBucketVector* arr, const CacheLruListHashEntry* obj, size_t count) {
	if (arr->capacity <= arr->count + count) {
		CacheLruListHashBucketVectorExpand(arr, count);
	}
	memcpy((void*)(&arr->obj_arr[arr->count]), (void*)(obj), (sizeof(CacheLruListHashEntry) * count));
	arr->count += count;
	return arr->count - count;
}
...
__forceinline CacheLruListHashLinkRbEntry* CacheLruListHashLinkRbReferencer_Reference(CacheLruListHashLinkRbTree* tree, int32_t entry_id) {
	if (entry_id == CacheLruListHashLinkRbReferencer_InvalidId) return ((void*)0);
...
_Bool CacheLruListHashLinkRbBsTreeInsert(CacheLruListHashLinkRbBsTree* tree, int32_t entry_id) {
	CacheLruListHashLinkRbBsEntry* entry = CacheLruListHashLinkRbReferencer_Reference(tree, entry_id);
	...
	CacheLruListHashLinkRbBsEntry* cur = ((void*)0);
	_Bool success = 1;
	while (cur_id != CacheLruListHashLinkRbReferencer_InvalidId) {
		...
		PageId* cur_key = CacheLruListHashLinkRbAccessor_GetKey(tree, cur);
		PageId* entry_key = CacheLruListHashLinkRbAccessor_GetKey(tree, entry);
	...
	if (cur) CacheLruListHashLinkRbReferencer_Dereference(tree, cur);
	if (cur_id != entry_id) {
		CacheLruListHashLinkRbBsEntryInit(tree, entry);
...
int32_t CacheLruListHashLinkRbBsTreePut(CacheLruListHashLinkRbBsTree* tree, int32_t entry_id) {
	CacheLruListHashLinkRbBsEntry* entry = CacheLruListHashLinkRbReferencer_Reference(tree, entry_id);
	if (tree->root == CacheLruListHashLinkRbReferencer_InvalidId) {
		CacheLruListHashLinkRbBsEntryInit(tree, entry);
		tree->root = entry_id;
	...
	int32_t cur_id = tree->root;
	CacheLruListHashLinkRbBsEntry* cur = ((void*)0);
	int32_t old_id = CacheLruListHashLinkRbReferencer_InvalidId;
	while (cur_id != CacheLruListHashLinkRbReferencer_InvalidId) {
		...
		if ((*(cur_key) < *(entry_key))) {
			...
			*entry = *cur;
			CacheLruListHashLinkRbAccessor_SetParent(tree, entry, parent_id);
			if (cur->left != CacheLruListHashLinkRbReferencer_InvalidId) {
				CacheLruListHashLinkRbBsEntry* left = CacheLruListHashLinkRbReferencer_Reference(tree, cur->left);
				CacheLruListHashLinkRbAccessor_SetParent(tree, left, entry_id);
				CacheLruListHashLinkRbReferencer_Dereference(tree, left);
			}
			if (cur->right != CacheLruListHashLinkRbReferencer_InvalidId) {
				CacheLruListHashLinkRbBsEntry* right = CacheLruListHashLinkRbReferencer_Reference(tree, cur->right);
				CacheLruListHashLinkRbAccessor_SetParent(tree, right, entry_id);
				CacheLruListHashLinkRbReferencer_Dereference(tree, right);
			}
			break;
		}
		CacheLruListHashLinkRbReferencer_Dereference(tree, cur);
	...
	if (cur) CacheLruListHashLinkRbReferencer_Dereference(tree, cur);
	if (old_id == CacheLruListHashLinkRbReferencer_InvalidId) {
		CacheLruListHashLinkRbAccessor_SetParent(tree, entry, cur_id);
	}
	CacheLruListHashLinkRbReferencer_Dereference(tree, entry);
	return old_id;
...
_Bool CacheLruListHashLinkRbTreeInsert(CacheLruListHashLinkRbTree* tree, int32_t insert_entry_id) {
	if (!CacheLruListHashLinkRbBsTreeInsert(&tree->bs_tree, insert_entry_id)) return 0;
	CacheLruListHashLinkRbTreeInsertFixup(tree, insert_entry_id);
	return 1;
}
int32_t CacheLruListHashLinkRbTreePut(CacheLruListHashLinkRbTree* tree, int32_t put_entry_id) {
	int32_t old_id = CacheLruListHashLinkRbBsTreePut(&tree->bs_tree, put_entry_id);
	if (old_id == CacheLruListHashLinkRbReferencer_InvalidId) CacheLruListHashLinkRbTreeInsertFixup(tree, put_entry_id);
	return old_id;
}
...
static __forceinline uint32_t CacheLruListHashGetIndex(CacheLruListHashTable* table, const PageId* key) {
	return Hashmap_hashint(*key) % table->bucket.capacity;
}
...
CacheLruListEntry* CacheLruListPut(CacheLruList* list, CacheLruListEntry* entry) {
	PageId* key = (&((CacheInfo*)((uintptr_t)(entry)-((uintptr_t) & (((CacheInfo*)0)->lru_entry))))->pgid);
	CacheLruHashEntry* hash_entry = CacheLruListHashTableFind(&list->hash_table, key);
	if (hash_entry) {
...
CacheLruListEntry* CacheLruListPop(CacheLruList* list) {
	...
	PageId* key = (&((CacheInfo*)((uintptr_t)((CacheLruListEntry*)del_list_entry) - ((uintptr_t) & (((CacheInfo*)0)->lru_entry))))->pgid);
...
CacheLruListEntry* CacheLruListIteratorLast(CacheLruList* list) {
	ListEntry* list_entry = ListLast(&list->list_head);
	return (CacheLruListEntry*)list_entry;
}
...
void CacherFree(Cacher* cacher, CacheId cache_id) {
	...
	RwLockWriteAcquire(&cacher->hotspot_queue_lock);
	CacheLruListDelete(&cacher->lru_list, &cache_info->pgid);
